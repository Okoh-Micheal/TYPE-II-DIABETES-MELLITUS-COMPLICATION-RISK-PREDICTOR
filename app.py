...
if st.button("🔍 Generate Risk Prediction", use_container_width=True):
    ...
    # ── SHAP values via XGBoost native pred_contribs ──────────────────────────
    # This completely bypasses the shap library and its XGBoost version conflicts.
    # XGBoost's pred_contribs=True returns the same SHAP values natively,
    # with zero dependency on shap internals or base_score formatting.
    # ─────────────────────────────────────────────────────────────────────────
    ...
    # Build DMatrix with feature names so XGBoost knows the column order
    dmatrix = xgb.DMatrix(
        transformed_patient,
    ...
    contribs = model_internal.get_booster().predict(dmatrix, pred_contribs=True)
    sv = contribs[0][:-1]  # drop bias column, keep feature contributions
